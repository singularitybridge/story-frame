/**
 * Add Sofia character to the bedroom image (crop to portrait and composite)
 */
...
const locationImagePath = path.join(process.env.HOME, 'Downloads', '739405170.jpg');
const outputDir = path.join(process.cwd(), 'public', 'generated-refs', projectId);
const outputPath = path.join(outputDir, 'character-ref-portrait-2.png');

async function processBedroomImage() {
  console.log('Processing bedroom image...\\n');
  ...
  console.log('Adding Sofia to the bedroom scene...\\n');

  const ai = new GoogleGenAI({ apiKey });

  // Use Gemini to add Sofia to the cropped image
  const prompt = `Add ${characterDescription} standing in this beautiful heritage bedroom, positioned on the left side near the ornate wooden bed, looking at camera with warm welcoming smile, natural pose showing the beautiful room, photorealistic, high quality, natural lighting from the window`;
  ...
  console.log(`✓ Saved: character-ref-portrait-2.png`);
  console.log(`Location: ${outputPath}`);
}

processBedroomImage().catch(error => {
